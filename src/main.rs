...
fn main() { // main program entry point, fn with no parameters
    ...
    loop {
        ...
        // Important - switching from a expect call (below) to using a match expression is generally how you move from crashing on an error to handling it.
        // Parse returns a 'Result' type and which is an enum that has variants Ok and Err so you can match against them.
        // If Result Ok happens (parse can turn the string into a number) then it's going to just return the num value as guess.
        // If Result Err happens it returns a catch all (_) error value with info about the error and 'continue' to the next iteration of the loop and ask for another guess
        
        // above block did look like this
        ...
        match guess.cmp(&secret_number) {
            ...
            Ordering::Equal => {
                println!("YES! YOU WIN");
                break; // exits the program nicely here when the guess is made correctly
            }
        }
    } // end of loop - if all you do is add a loop then the only way to exit the program is to make it fail by entering a non-number...
}